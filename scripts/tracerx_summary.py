...
def get_tallies(out_dir, program):
    tracerx_out_dir = os.path.join(out_dir, "tracerx-out")
    ...
    try:
        f = open(os.path.join(tracerx_out_dir, "run.stats"))
        run_stats = f.read().strip()
        f.close()
        run_stats = list(map(lambda x: x.strip().strip("(),"), run_stats.split("\n")))
        ...
        with open(os.path.join(tracerx_out_dir, "info"), "r") as f:
            lines = f.readlines()
        start = None
        end = None
        for line in lines:
    ...
    tests_generated = sum(file.endswith(".ktest") for file in os.listdir(tracerx_out_dir))
    assertion_errors = sum(file.endswith(".assert.err") for file in os.listdir(tracerx_out_dir))

    return (fully_explored_branches, total_branches, assertion_errors, not has_error, elapsed)

if __name__ == "__main__":
    tracerx_out_dir = os.path.join(os.path.dirname(os.getcwd()), "out")
    program = "Prob16-R12-B3.c"

    print(get_tallies(tracerx_out_dir, program))
